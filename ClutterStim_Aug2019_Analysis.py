...
filedir = '../../RawData/';
filename = 'F190809_ClutterStim_080819_0.5RF_recut.nex';
prevTime = 0.3;
numStims = 63*15;  ## stimuli changed depending on repetition numbers
                   ## so number of condition (63) * number of repetition (15)
...
12 Near Small Circle Surround alone (54)
Center + NearSmallCircleSurround (55-62)  
"""
...
for j in np.arange(experiment['numNeurons']):
    ...
    cNSC_ste = [];
    for i in np.arange(8):
        cNSC_ste.append((np.std(StimResp[55+i]['neurons'][j]['numspikes'])*1000/350)
                            /(np.sqrt(len(StimResp[55+i]['neurons'][j]['numspikes']))));
    plt.errorbar(np.arange(1,9),cAlone[rk],np.array(cAlone_ste)[rk],color=[1,0,0]);
    plt.errorbar(np.arange(1,9),cNSC[rk],np.array(cNSC_ste)[rk],color=[0,0,1]);    
