...
class ClutterStim:
    ...
    def __init__(self, app):
        ...
        self.mySurrNearSmallCircleSprites = list();            
        self.myid = list();
        self.numStim = 63*15;
    ...
    def createStimuli(self,app):
        ...
        # 12 Small Circle Near Surround alone (55)        
        # Center + 12 Small Circle Near Surround (56-63)          
        
        repetitionIDs = np.arange(15); ### inter-block order    
        if(randomize_stims):
            np.random.shuffle(repetitionIDs);
        rOrder = np.arange(63); ### inner-block order
        StimInfoTable = np.empty((0,9),int);
        for r in np.arange(15):
            rNow = repetitionIDs[r];
            StimInfoTableNow = np.zeros((63,9),dtype=int);
            StimInfoTableNow[:,0] = rNow*63 + np.arange(1,64,1,dtype=int);
            # assign Center
            ...
            # assign SurrSC            
            StimInfoTableNow[54:63,7] = rNow+1;    

            # assign random order
            if(randomize_stims):            
                np.random.shuffle(rOrder);
            StimInfoTableNow[:,8] = rOrder;
            StimInfoTableNow = StimInfoTableNow[
                    StimInfoTableNow[:,8].argsort()];
        ...
        MatFileName = os.path.join(MatFileBase, 'ClutterStim3_0.5.mat');            
        ...
        # SurrNearSmallCircle
        for i in range(15):
            ref = sData['SurrNearSmallCircle']['Img'][i,0];
            SpriteSize = np.shape(sData[ref].value)[1];
            ...
            myScale = int(RF_dia*SpriteSize/100)
            s.scale(myScale, myScale)    #Scales to size in pixels		   
            self.mySurrNearSmallCircleSprites.append(s)
        print('SurrNearSmallCircle sprites were created');       
    ...
    def _RunTrial(self,app, t):
        ...
        for i in arange(0,sshow):
            ...
            if spr[2]>0:
                ...
                Sstim = self.mySurrNearSmallCircleSprites[spr[7]-1];         
