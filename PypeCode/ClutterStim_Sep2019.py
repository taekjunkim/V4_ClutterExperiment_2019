...
class ClutterStim:
    ...
    def __init__(self, app):
        ...
        self.myGrayCenterSprites = list(); 
        self.myColorCenterSprites = list();         
        ...
        self.mySurr6NearCircleSprites = list();                        
        self.mySurr12NearSmallSprites = list();                                
    ...
    def createStimuli(self,app):
        ...
        ### StimInfoTable
        # columns: Trial#, Center, Surr1, Surr3, Surr6, SurrM, SurrF, SurrSC, RndN
        # No stim (1)
        # Gray Center alone (2-9)
        # near 1 Surround alone (10)
        # Gray Center + 1 Surround (11-18)
        # near 3 Surround alone (19)
        # Gray Center + 3 Surround (20-27)
        # near 6 Surround alone (28)
        # Gray Center + 6 Surround (Near) (29-36)
        # middle 12 Surround alone (37)
        # Gray Center + 12 Surround (Middle) (38-45)
        # far 18 Surround alone (46)
        # Gray Center + 18 Surround (Far) (47-54)  
        # 6 Circle Near Surround alone (55)        
        # Gray Center + 6 Circle Near Surround (56-63)          
        # 12 Small Near Surround alone (64)        
        # Gray Center + 12 Small Near Surround (65-72)          
        # Color Center alone (73-80)                  
        # Color Center + 6 Surround (Near) (81-88)                          
        # Gray Center + 6 Surround (PS) (89-96)                                  
        # Gray Center + 6 Circle Near Surround (PS) (97-104)                                          
        # Gray Center + 12 Small Near Surround (PS) (105-112)
        
        repetitionIDs = np.arange(10); ### inter-block order    
        if(randomize_stims):
            np.random.shuffle(repetitionIDs);
        rOrder = np.arange(112); ### inner-block order
        StimInfoTable = np.empty((0,12),int);
        """
        ...
        col6: middle surr 12                        
        col7: far surr 18
        col8: near surr circle 6
        col9: near surr small 12
        col10: PS version
        col11: Shuffled order
        """
        for r in np.arange(10): # 10 repetition
            rNow = repetitionIDs[r];
            StimInfoTableNow = np.zeros((112,12),dtype=int);
            StimInfoTableNow[:,0] = rNow*112 + np.arange(1,112+1,1,dtype=int);
            ...
            StimInfoTableNow[88:96,1] = np.arange(1,9,1,dtype=int);               
            StimInfoTableNow[96:104,1] = np.arange(1,9,1,dtype=int);                                                   
            StimInfoTableNow[104:112,1] = np.arange(1,9,1,dtype=int);                                                               
            # assign color center
            StimInfoTableNow[72:80,2] = np.arange(1,9,1,dtype=int);           
            StimInfoTableNow[80:88,2] = np.arange(1,9,1,dtype=int);   
            # assign near surr 1            
            ...
            StimInfoTableNow[80:96,5] = rNow+1;                 
            # assign middle surr 12            
            StimInfoTableNow[36:45,6] = rNow+1;     
            # assign far surr 18            
            StimInfoTableNow[45:54,7] = rNow+1;    
            # assign near surr circle 6            
            StimInfoTableNow[54:63,8] = rNow+1;    
            StimInfoTableNow[96:104,8] = rNow+1;                
            # assign near surr small 12                        
            StimInfoTableNow[63:72,9] = rNow+1;    
            StimInfoTableNow[104:112,9] = rNow+1;                
            # assign PS verson
            StimInfoTableNow[88:112,10] = rNow+1;                            

            # assign random order
            if(randomize_stims):            
                np.random.shuffle(rOrder);
            StimInfoTableNow[:,11] = rOrder;
            StimInfoTableNow = StimInfoTableNow[
                    StimInfoTableNow[:,11].argsort()];
        ...
        MatFileName = os.path.join(MatFileBase, 'ClutterStim4_0.5_cropped_uint8.mat');            
        ...
        for j in range(0,len(RotationIDs)):
            ...
            self.myGrayCenterSprites.append(s);                    
            s2.alpha[:,:] = ForAlpha.array[:,:,0];
            self.myColorCenterSprites.append(s2);                    
        ...
        for i in range(10):
            ...
            myScale = int(RF_dia*SpriteSize/100)
            s.scale(myScale, myScale)    #Scales to size in pixels		   
            self.mySurr6NearCircleSprites.append(s)
        ...
        for i in range(10):
            ...
            myScale = int(RF_dia*SpriteSize/100)
            s.scale(myScale, myScale)    #Scales to size in pixels		   
            self.mySurr12NearSmallSprites.append(s)
...
#                self.mySprites[i].__del__()
    ...
    def _RunTrial(self,app, t):
        ...
        for i in arange(0,sshow):
            ...
            if spr[10]==0: #non-PS version
                # Center Sprite
                if spr[1]>0:
                    Cstim = self.myGrayCenterSprites[spr[1]-1];
                elif spr[2]>0:    
                    Cstim = self.myColorCenterSprites[spr[2]-1];					
                # Surround Sprite
                if spr[3]>0:
                    Sstim = self.mySurr1NearSprites[spr[3]-1];
                elif spr[4]>0:
                    Sstim = self.mySurr3NearSprites[spr[4]-1];         
                elif spr[5]>0:
                    Sstim = self.mySurr6NearSprites[spr[5]-1];         
                elif spr[6]>0:
                    Sstim = self.mySurrMiddleSprites[spr[6]-1];         
                elif spr[7]>0:
                    Sstim = self.mySurrFarSprites[spr[7]-1];         
                elif spr[8]>0:
                    Sstim = self.mySurr6NearCircleSprites[spr[8]-1];         
                elif spr[9]>0:
                    Sstim = self.mySurr12NearSmallSprites[spr[9]-1];         
            else:
                # Center Sprite
                if spr[5]>0:
					Cstim = self.myGrayCenter6SurrPS_Sprites[spr[5]-1][spr[1]-1];
                elif spr[8]>0:
					Cstim = self.myGrayCenter6CircleSurrPS_Sprites[spr[8]-1][spr[1]-1];
                elif spr[9]>0:
                    Cstim = self.myGrayCenter12SmallSurrPS_Sprites[spr[9]-1][spr[1]-1];         
